...
bool ManagedFileBasedContainer<T, Access>::saveManagedObjectToFile(
  ...
  // if not overwrite, then attempt to find a non-conflicting name
  bool found = true;
  int count = 0;
  while (found) {
    found = FileUtil::exists(FileUtil::join(fileDirectory, fileName));
    if (found) {
      // build a new file name candidate by adding copy plus some integer value
      fileName = Cr::Utility::formatString("{} (copy {:04d}).{}", fileNameBase,
                                           count, this->JSONTypeExt_);
      ++count;
    }
  }
  return this->saveManagedObjectToFileInternal(obj, fileName, fileDirectory);
  // const std::string fileNameCandidate = obj->get
